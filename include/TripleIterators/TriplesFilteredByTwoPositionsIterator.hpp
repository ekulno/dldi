#ifndef DLDI_TRIPLES_FILTERED_BY_TWO_POSITIONS_ITERATOR_HPP
#define DLDI_TRIPLES_FILTERED_BY_TWO_POSITIONS_ITERATOR_HPP
...
namespace dldi {
  class TriplesFilteredByTwoPositionsIterator : public Iterator<QuantifiedTriple> {
  public:
    TriplesFilteredByTwoPositionsIterator(
      const dldi::TripleOrder& order,
      const std::size_t& primary_id,
      const std::size_t& secondary_id,
...

    {
      ...
      const auto primary_ids_index{BinarySearch::binary_search(
      ...
      m_secondary_ids_index = BinarySearch::binary_search(
        std::get<0>(secondary_ids),
        std::get<0>(secondary_refs)[primary_ids_index],
        std::get<0>(secondary_refs)[primary_ids_index + 1] - 1,
        secondary_id,
        dicts->get_dict(secondary_position));
